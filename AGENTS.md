...
After editing any `.css` or `.js` under `includes/admin/`, run `pnpm run build:assets` to regenerate the `.min` and RTL files.
...
`install.sh` uses GNU `sed`; on macOS its final rewrite of `wp-tests-config.php` fails, requiring the ABSPATH and DB lines to be filled in by hand. It works unmodified in CI, so don't run PHPUnit locally — `.github/workflows/unit-tests.yml` runs it on every push, and that's sufficient.

The WordPress test suite does not support PHPUnit 10+ (`PHPUnit\Util\Test::parseTestMethodAnnotations` was removed); run PHPUnit 9 locally, which `.github/workflows/unit-tests.yml` also pins. Test files are named for their class (`ConverterTest.php` → `ConverterTest`) because PHPUnit 10+ requires the match.
...
- **`Capabilities` probes, it does not trust.** `Imagick::queryFormats()` reporting AVIF means a delegate is registered, not that encoding works. Each driver/format pair is verified by actually encoding a bundled 64×64 PNG, cached in the `wzio_capabilities` option keyed on plugin version.
- **`wp_content_img_tag` often passes `$attachment_id === 0`** — the ID comes from the `wp-image-{ID}` class, absent from content from other editors and older imports. `Frontend\Resolver` is therefore keyed on URL, not attachment ID, with a request memo plus object cache (24h for hits, 5min for misses).
- **`srcset` descriptors are copied verbatim**, never re-derived. A partial optimized `<source>` may omit missing intermediate candidates, but the smallest and widest or highest-density candidates must resolve so it does not over-download or reduce the original set's maximum resolution. Complete formats are listed before partial formats. Mixed, bare and unrecognized descriptor sets retain the all-candidates rule.
...
- **A sidecar that is not smaller than its source is deleted**, and the skip recorded per file, which is what keeps `file_exists()` a self-consistent serving decision.
